...
export default function Dashboard({ auth }) {
    ...
    const [showComplianceFilter, setShowComplianceFilter] = useState(false);
    const [filterStartDate, setFilterStartDate] = useState('');
    const [filterEndDate, setFilterEndDate] = useState('');
    const [filteredComplianceStats, setFilteredComplianceStats] = useState(null);
    ...
    // Filter compliance data based on date range
    const applyComplianceFilter = async () => {
        if (!filterStartDate && !filterEndDate) {
            setFilteredComplianceStats(null);
            return;
        }
        ...
        try {
            ...
            // Make API call to get real compliance data
            const response = await fetch(`/admin/dashboard/compliance-filter?${params.toString()}`, {
                headers: {
            ...
            let compliantCount = stats.compliantEstablishments;
            let nonCompliantCount = stats.nonCompliantEstablishments;
            ...
            if (filterStartDate && filterEndDate) {
                ...
                if (daysDiff > 0) {
                    const rangeMultiplier = Math.max(0.2, Math.min(1, daysDiff / 90));
                    compliantCount = Math.round(compliantCount * rangeMultiplier);
                    nonCompliantCount = Math.round(nonCompliantCount * rangeMultiplier);
                    
                    if (compliantCount === 0) compliantCount = 1;
                    if (nonCompliantCount === 0) nonCompliantCount = 1;
                }
            }
    ...
    const resetComplianceFilter = () => {
        setFilterStartDate('');
        setFilterEndDate('');
        setFilteredComplianceStats(null);
    };

    // Get current compliance stats (filtered or original)
    const getCurrentComplianceStats = () => {
        if (filteredComplianceStats) {
            return {
                compliant: filteredComplianceStats.compliant,
                nonCompliant: filteredComplianceStats.nonCompliant,
                total: filteredComplianceStats.total
            };
        }
        return {
            compliant: stats.compliantEstablishments,
            nonCompliant: stats.nonCompliantEstablishments,
            total: stats.totalEstablishments
        };
    };
    return (
        <AuthenticatedLayout
                        ...
                        <div className="relative p-6">
                            ...
                            <div>
                                ...
                                <p className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                                    {Math.round((getCurrentComplianceStats().compliant / getCurrentComplianceStats().total) * 100)}%
                                </p>
                                <div className="flex items-center text-xs text-gray-500 dark:text-gray-400">
                                    <FireIcon className="w-4 h-4 text-amber-500 mr-1" />
                                    <span>Excellent progress</span>
                                    {filteredComplianceStats && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-8 hover:shadow-2xl transition-all duration-500">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <div className="flex items-center space-x-2">
                                <button
                                    onClick={() => setShowComplianceFilter(!showComplianceFilter)}
                                    className={`p-2 rounded-xl transition-all duration-300 ${
                                        showComplianceFilter || filteredComplianceStats
                                            ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600'
                                            : 'bg-gray-100 dark:bg-gray-700 text-gray-600 hover:bg-gray-200 dark:hover:bg-gray-600'
                                    }`}
                                    title="Filter Compliance Data"
                                >
                            ...
                            <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-xl border border-blue-200 dark:border-blue-700">
                                <div className="flex items-center justify-between mb-4">
                                    <h4 className="text-sm font-bold text-gray-900 dark:text-white flex items-center space-x-2">
                                        ...
                                        <span>Filter Compliance Data</span>
                                    </h4>
                                    <button
                                        onClick={() => setShowComplianceFilter(false)}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full text-sm border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                                        />
                                    </div>
                                    <div>
                                ...
                                <div className="flex items-center space-x-3">
                                    <button
                                        onClick={applyComplianceFilter}
                                        disabled={isLoadingCompliance}
                                        className="px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white text-sm font-semibold rounded-xl hover:from-blue-600 hover:to-purple-700 transition-all duration-300 shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
                                    >
                                        {isLoadingCompliance ? (
                                            <>
                                    ...
                                    <button
                                        onClick={resetComplianceFilter}
                                        disabled={isLoadingCompliance}
                                        className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-semibold rounded-xl hover:bg-gray-300 dark:hover:bg-gray-600 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        Reset
                                    </button>
                                    {filteredComplianceStats && (
                                        <div className="flex items-center space-x-1 bg-green-100 dark:bg-green-900/30 px-3 py-1 rounded-full">
                                            ...
                                            <span className="text-xs font-bold text-green-600">Filter Active</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="group">
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <span className="text-sm font-bold text-green-600 bg-green-100 dark:bg-green-900/30 px-3 py-1 rounded-full">
                                        {getCurrentComplianceStats().compliant}
                                    </span>
                                </div>
                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 overflow-hidden">
                                    <div 
                                        ...
                                        style={{ width: `${(getCurrentComplianceStats().compliant / getCurrentComplianceStats().total) * 100}%` }}
                                    >
                                        <div className="absolute inset-0 bg-white opacity-20 animate-pulse"></div>
                                    </div>
                                </div>
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                                    {Math.round((getCurrentComplianceStats().compliant / getCurrentComplianceStats().total) * 100)}% of total establishments
                                </p>
                            </div>
                            <div className="group">
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <span className="text-sm font-bold text-red-600 bg-red-100 dark:bg-red-900/30 px-3 py-1 rounded-full">
                                        {getCurrentComplianceStats().nonCompliant}
                                    </span>
                                </div>
                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 overflow-hidden">
                                    <div 
                                        ...
                                        style={{ width: `${(getCurrentComplianceStats().nonCompliant / getCurrentComplianceStats().total) * 100}%` }}
                                    >
                                        <div className="absolute inset-0 bg-white opacity-20 animate-pulse"></div>
                                    </div>
                                </div>
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                                    {Math.round((getCurrentComplianceStats().nonCompliant / getCurrentComplianceStats().total) * 100)}% require attention
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="relative z-10 flex items-center justify-between">
                            <div>
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    {filteredComplianceStats && (
                                        <span className="ml-2 bg-white/20 px-2 py-1 rounded-full text-xs">Filtered</span>
                                    )}
                                </div>
                            </div>
                            <div className="relative">
